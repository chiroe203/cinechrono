...
| ファイル | 変更内容 |
|---------|---------|
| `api/translate.js` | 🆕 Vercel Functions（DeepL翻訳プロキシ） |
| `src/libs/deepl.js` | 🆕 DeepL翻訳API接続（Vercel Functions経由） |
| `src/libs/rawg.js` | ✅ 変更不要（既にdescription取得対応済み） |
| `src/components/admin/AdminPanel.jsx` | ゲームあらすじ編集欄、一括更新（あらすじなしのみ）、初期選択対応 |
| `src/components/modals/DetailModal.jsx` | ゲームの`translatedDescription`を「あらすじ」として表示 |
| `src/App.js` | DeepLインポート、bulkUpdateGameSynopsis関数、translatedDescription対応、initialSelectedGame対応 |
...
### ゲームあらすじ編集欄
- ゲームを選択すると、黄色の編集エリアが表示
- `translatedDescription`を直接編集可能
- 保存するとFirebaseに反映

### 一括更新機能
- 「あらすじなしのゲームを一括取得」ボタン
- あらすじがないゲームのみ選択対象
- すべてのゲームにあらすじがある場合は「✅ すべてのゲームにあらすじが登録されています」と表示

### 詳細モーダルからの編集
- 詳細モーダルで「編集」ボタンをクリック
- 管理画面が開き、そのゲームにデフォルトでチェックが入る
- すぐに「あらすじを取得」ができる状態に
...
| Key | Value | Note |
|-----|-------|------|
| `DEEPL_API_KEY` | `f4e2412f-9ab7-4db9-beb1-878f33abce63:fx` | DeepL翻訳API（サーバーサイド用） |

⚠️ **注意**: `REACT_APP_DEEPL_API_KEY` ではなく `DEEPL_API_KEY` を使用（Vercel Functions用）
...
3. 「あらすじなしを全選択」ボタンで一括選択
4. 「あらすじなしのゲームを一括取得」ボタンをクリック
5. 進捗バーで処理状況を確認
6. 完了後、結果がアラートで表示される

### 詳細モーダルから個別に更新する場合

1. 年表でゲームをクリック → 詳細モーダルを開く
2. 「編集」ボタンをクリック
3. 管理画面が開き、そのゲームにチェックが入った状態に
4. 「あらすじなしのゲームを一括取得」ボタンをクリック
...
- **英語タイトルが必須**: RAWGは英語タイトルで検索するため、英語タイトルが未入力のゲームは自動取得できません
- **ひとことTipsは別フィールド**: あらすじ（translatedDescription）と💡ひとことTips（synopsis）は別々に管理されます
...
| Key | Value |
|-----|-------|
| `DEEPL_API_KEY` | `f4e2412f-9ab7-4db9-beb1-878f33abce63:fx` |

⚠️ **重要**: `REACT_APP_` プレフィックスは不要です（サーバーサイドで使用するため）
